...
export class SnapProvider {
   ...
   * SnapProvider appends a "View in dashboard →" link when a run ID is available.
   *
  ...
  buildCommentBody(summary, meta = {}) {
    const dashboardUrl = this.#lastRunId
      ? `${this.#apiUrl}/projects/${this.#projectId}/runs/${this.#lastRunId}`
      : undefined;
    return buildReportCommentBody(summary, { ...meta, dashboardUrl });
  }

  /** @internal Test-only hook to set the last run ID. */
  _testSetLastRunId(runId) {
    this.#lastRunId = runId;
  }
