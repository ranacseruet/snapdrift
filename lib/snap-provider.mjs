...
import { COMPARISON_FALLBACK_REASONS, COMPARISON_ROW_KINDS } from '@snapdrift/compare-core';
import {
...
/** @typedef {import('../types/visual-diff-types').ComparisonPolicy} ComparisonPolicy */
/** @typedef {import('../types/visual-diff-types').ComparisonMetadata} ComparisonMetadata */
/** @typedef {import('../types/visual-diff-types').JsonObject} JsonObject */
/** @typedef {{ deadline: number, label: string }} OperationDeadline */
...
const JSON_REQUEST_TIMEOUT_MS = 30 * 1000;
const BINARY_REQUEST_TIMEOUT_MS = 120 * 1000;
const POLL_INTERVAL_MS = 2000;
const MAX_OPERATION_DURATION_MS = 10 * 60 * 1000;
...
const OPERATION_LIMIT_TEXT = formatDuration(MAX_OPERATION_DURATION_MS);
...
/**
 * Run an asynchronous operation with a timer that also settles when the
 * underlying promise ignores cancellation. The promise remains observed after
 * the timeout, so a late rejection cannot become an unhandled rejection.
 *
 * @template T
 ...
 * @returns {Promise<T>}
 */
function runWithTimeout(task, timeoutMs, onTimeout, timeoutError) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(
      () => {
        ...
        try {
          ...
          // Abort/cancel is best effort; the timeout still settles the caller.
        }
        reject(timeoutError());
      },
    ...
    taskPromise.then(
      (value) => {
        if (settled) {
          if (value && typeof value === 'object') cancelResponseBody(value);
          return;
        }
        settled = true;
        clearTimeout(timer);
...
/** @param {unknown} value @returns {value is ComparisonPolicy} */
function isComparisonPolicy(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const policy = /** @type {Record<string, any>} */ (value);
  return (policy.version === 1 || policy.version === 2) && typeof policy.threshold === 'number' && Number.isFinite(policy.threshold) && policy.threshold >= 0 && policy.threshold <= 1;
}

/** @param {unknown} value @returns {value is ComparisonMetadata} */
function isComparisonMetadata(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const comparison = /** @type {Record<string, any>} */ (value);
  const dimensions = [comparison.baseline, comparison.current, comparison.canvas];
  if (
    dimensions.some(
      (dimension) =>
        !dimension ||
        typeof dimension !== 'object' ||
        !Number.isInteger(dimension.width) ||
        dimension.width < 0 ||
        !Number.isInteger(dimension.height) ||
        dimension.height < 0
    )
  ) {
    return false;
  }
  const [baseline, current, canvas] = dimensions;
  const baseMetadataValid = (
    typeof comparison.dimensionsChanged === 'boolean' &&
    comparison.dimensionsChanged === (baseline.width !== current.width || baseline.height !== current.height) &&
    Number.isInteger(comparison.totalPixels) &&
    comparison.totalPixels >= 0
  );
  if (!baseMetadataValid) return false;
  if (comparison.policyVersion !== undefined && comparison.policyVersion !== 2) return false;

  if (comparison.policyVersion !== 2) {
    if (comparison.mode !== undefined || comparison.rowMapping !== undefined || comparison.fallbackReason !== undefined) {
      return false;
    }
    return (
      canvas.width === Math.max(baseline.width, current.width) &&
      canvas.height === Math.max(baseline.height, current.height) &&
      comparison.totalPixels <= canvas.width * canvas.height
    );
  }

  if (comparison.mode === 'coordinate-fallback') {
    return (
      comparison.rowMapping === undefined &&
      COMPARISON_FALLBACK_REASONS.includes(comparison.fallbackReason) &&
      canvas.width === Math.max(baseline.width, current.width) &&
      canvas.height === Math.max(baseline.height, current.height) &&
      comparison.totalPixels <= canvas.width * canvas.height
    );
  ...
  if (comparison.mode !== 'vertical-aligned' || comparison.fallbackReason !== undefined || !Array.isArray(comparison.rowMapping) || baseline.width !== current.width || canvas.width !== baseline.width) {
    return false;
  }
  let outputStart = 0;
  let baselineStart = 0;
  let currentStart = 0;
  for (const segment of comparison.rowMapping) {
    if (!segment || !COMPARISON_ROW_KINDS.includes(segment.kind) || !Number.isInteger(segment.outputStart) || segment.outputStart !== outputStart || !Number.isInteger(segment.length) || segment.length <= 0) {
      return false;
    }
    const hasBaseline = segment.kind === 'matched' || segment.kind === 'changed' || segment.kind === 'deleted';
    const hasCurrent = segment.kind === 'matched' || segment.kind === 'changed' || segment.kind === 'inserted';
    if (hasBaseline !== (Number.isInteger(segment.baselineStart)) || hasCurrent !== (Number.isInteger(segment.currentStart))) {
      return false;
    }
    if (
      (hasBaseline && segment.baselineStart !== baselineStart) ||
      (!hasBaseline && segment.baselineStart !== undefined) ||
      (hasCurrent && segment.currentStart !== currentStart) ||
      (!hasCurrent && segment.currentStart !== undefined)
    ) {
      return false;
    }
    if (hasBaseline && (segment.baselineStart < 0 || segment.baselineStart + segment.length > baseline.height)) return false;
    if (hasCurrent && (segment.currentStart < 0 || segment.currentStart + segment.length > current.height)) return false;
    if (segment.comparedOffset !== undefined) {
      if (!hasBaseline || !hasCurrent || (segment.comparedOffset !== -1 && segment.comparedOffset !== 0 && segment.comparedOffset !== 1)) {
        return false;
      }
      const comparedStart = segment.currentStart + segment.comparedOffset;
      if (comparedStart < 0 || comparedStart + segment.length > current.height) return false;
    }
    outputStart += segment.length;
    if (hasBaseline) baselineStart += segment.length;
    if (hasCurrent) currentStart += segment.length;
  }
  return canvas.height === outputStart && baselineStart === baseline.height && currentStart === current.height && canvas.height >= Math.max(baseline.height, current.height) && canvas.height <= baseline.height + current.height && comparison.totalPixels === canvas.width * canvas.height;
}

/**
 * @param {any} capture
 * @param {ComparisonMetadata} comparison
 * @param {number} expectedThreshold
 * @returns {boolean}
 */
function hasCompleteComparisonResult(capture, comparison, expectedThreshold) {
  if (
    !Number.isFinite(capture.diffPct) ||
    capture.diffPct < 0 ||
    capture.diffPct > 1 ||
    !Number.isInteger(capture.diffPixels) ||
    capture.diffPixels < 0 ||
    capture.diffPixels > comparison.totalPixels ||
    typeof capture.diffObjectKey !== 'string' ||
    capture.diffObjectKey.trim().length === 0 ||
    capture.thresholdUsed !== expectedThreshold
  ) {
    return false;
  }

  const expectedRatio = comparison.totalPixels === 0 ? 0 : capture.diffPixels / comparison.totalPixels;
  return Math.abs(capture.diffPct - expectedRatio) <= 1e-12;
}

/** @param {any} capture @returns {ComparisonMetadata | undefined} */
function comparisonFromCapture(capture) {
  const candidate =
    capture?.comparison ??
    (() => {
      if (typeof capture?.comparisonJson !== 'string') return undefined;
      try {
        return JSON.parse(capture.comparisonJson);
      } catch {
        return undefined;
      }
    })();
  return isComparisonMetadata(candidate) ? candidate : undefined;
}

/** @param {ComparisonMetadata} comparison @param {ComparisonPolicy} expected */
function comparisonMatchesPolicy(comparison, expected) {
  return expected.version === 2 ? comparison.policyVersion === 2 : comparison.policyVersion === undefined;
}

/** @param {unknown} actual @param {ComparisonPolicy} expected @param {string} context */
function assertComparisonPolicyAcknowledged(actual, expected, context) {
  if (!isComparisonPolicy(actual) || actual.version !== expected.version || actual.threshold !== expected.threshold) {
    throw new SnapUpgradeRequiredError(
      `${context} did not acknowledge comparisonPolicy v${expected.version} with threshold ${expected.threshold}. ` +
        'Upgrade the Snap server before enabling this comparison policy.'
    );
  }
}

/**
 * Updated SnapDrift clients explicitly opt into the union-canvas comparison.
 * Keep the config override for callers that want the policy visible in their
 * config, but derive it from the existing diff threshold by default so a
 * normal v1 client does not need to repeat that value.
 *
 * @param {VisualRegressionConfig} config
 * @returns {ComparisonPolicy}
 */
function resolveComparisonPolicy(config) {
  return config.diff.comparisonPolicy ? { ...config.diff.comparisonPolicy } : { version: 1, threshold: config.diff.threshold };
}
...
function resolveViewportDescriptor(viewport) {
  return typeof viewport === 'string' ? (VIEWPORT_PRESETS[viewport] ?? { width: 1280, height: 720 }) : viewport;
}
 ...
 * @returns {string}
 */
function normalizeViewportDescriptor(value, context, errorFactory) {
  let descriptor;
  try {
  ...
  if (!descriptor || typeof descriptor !== 'object' || Array.isArray(descriptor) || !Number.isFinite(descriptor.width) || !Number.isFinite(descriptor.height)) {
    throw errorFactory(`${context} has an invalid viewport descriptor.`);
  }
  return JSON.stringify(Object.fromEntries(Object.entries(descriptor).sort(([left], [right]) => (left < right ? -1 : left > right ? 1 : 0))));
}
 ...
 * @param {(detail: string) => Error} errorFactory
 */
function captureIdentityKey(capture, context, errorFactory) {
 ...
 * @returns {Set<string>}
 */
function uniqueRouteIdSet(value, field, errorFactory) {
 ...
 *   expectedCaptures: Array<{ routeId: string, routePath: string, viewportDescriptorJson: string, viewport: import('../types/visual-diff-types').VisualViewport }>,
 *   expectedByIdentity: Map<string, { routeId: string, routePath: string, viewportDescriptorJson: string, viewport: import('../types/visual-diff-types').VisualViewport }>,
 *   expectedByRouteId: Map<string, { routeId: string, routePath: string, viewportDescriptorJson: string, viewport: import('../types/visual-diff-types').VisualViewport }>
 * }}
 */
function validateHostedDiffMetadata(metadata, projectId) {
  ...
  if (metadata.projectId !== projectId) {
    throw hostedDiffError(`capture results belong to project "${metadata.projectId || 'unknown'}", not "${projectId}".`);
  }

  if (!Array.isArray(metadata.expectedCaptures) || metadata.expectedCaptures.length === 0) {
    throw hostedDiffError('capture results have no expected capture identities; recapture with SnapDrift 0.8.2 or newer.');
  }
  if (metadata.purpose !== 'diff') {
    throw hostedDiffError(`capture results have purpose "${metadata.purpose || 'unknown'}", not "diff"; ` + 'recapture with SnapDrift 0.8.2 or newer.');
  }
  ...
  if (metadata.expectedCaptures.length !== selected.size) {
    throw hostedDiffError(`capture results describe ${metadata.expectedCaptures.length} expected capture(s) for ${selected.size} selected route(s).`);
  }
  ...
  for (const [index, expected] of metadata.expectedCaptures.entries()) {
    if (
      ...
      typeof expected.routeId !== 'string' ||
      !expected.routeId ||
      typeof expected.routePath !== 'string' ||
      !expected.routePath ||
    ...
    const viewportDescriptorJson = normalizeViewportDescriptor(expected.viewportDescriptorJson, `expected capture "${expected.routeId}"`, hostedDiffError);
    const viewportDescriptor = JSON.parse(viewportDescriptorJson);
    const expectedCapture = {
      routeId: expected.routeId,
      routePath: expected.routePath,
      viewportDescriptorJson,
      viewport: { width: viewportDescriptor.width, height: viewportDescriptor.height }
    };
    ...
    expectedByRouteId.set(expected.routeId, expectedCapture);
  }
...
function validateCompleteBaselineRun(metadata, run, projectId) {
  ...
  if (typeof metadata.refBranch !== 'string' || !metadata.refBranch || typeof metadata.refSha !== 'string' || !metadata.refSha) {
    throw completeBaselineError('source results have no resolved ref; capture again with SnapDrift 0.8.2 or newer.');
  }
  if (typeof metadata.publicationSequence !== 'number' || !Number.isSafeInteger(metadata.publicationSequence) || metadata.publicationSequence <= 0) {
    throw completeBaselineError('source results have no valid publication sequence; capture again with SnapDrift 0.8.2 or newer.');
  }
  if (typeof metadata.publicationWorkflowRef !== 'string' || !metadata.publicationWorkflowRef.trim()) {
    throw completeBaselineError('source results have no publication workflow identity; capture again with SnapDrift 0.8.2 or newer.');
  }

  const configured = uniqueRouteIdSet(metadata.configuredRouteIds, 'configuredRouteIds', completeBaselineError);
  const selected = uniqueRouteIdSet(metadata.selectedRouteIds, 'selectedRouteIds', completeBaselineError);
  ...
  for (const expected of metadata.expectedCaptures) {
    if (
      ...
      typeof expected !== 'object' ||
      typeof expected.routePath !== 'string' ||
      !configured.has(expected.routeId) ||
      expectedRoutes.has(expected.routeId)
    ) {
    ...
    expectedByIdentity.set(captureIdentityKey(expected, `expected capture "${expected.routeId}"`, completeBaselineError), expected);
  }
  ...
  const routes = run.captures.map((capture) => {
    ...
    const identity = captureIdentityKey(capture, `source capture "${capture.routeId || capture.id || 'unknown'}"`, completeBaselineError);
  ...
  return routes.sort((left, right) => {
    const leftKey = captureIdentityKey(left, `route "${left.routeId}"`, completeBaselineError);
    const rightKey = captureIdentityKey(right, `route "${right.routeId}"`, completeBaselineError);
...
export function resolveGitRef() {
  ...
  return {
    refBranch: process.env.GITHUB_REF_NAME || process.env.GITHUB_HEAD_REF || (branch && branch !== 'HEAD' ? branch : undefined) || 'main',
...
function resolveHostedBaselinePublicationRef() {
  ...
  const publicationWorkflowRef = process.env.SNAPDRIFT_PUBLICATION_WORKFLOW_REF || process.env.GITHUB_WORKFLOW_REF;
...
export class SnapProvider {
  ...
  constructor(snapConfig, options = {}) {
    ...
    this.#nowFn = options.nowFn ?? monotonicNow;
  ...
  async capture(options) {
    ...
    const runContext = baselineRef ? { branch: baselineRef.refBranch, prHeadSha: baselineRef.refSha } : this.#gitRunContext();
    ...
    if (isLocalBaseUrl(config.baseUrl)) {
      return this.#captureLocalAndUpload({
        ...
        runContext,
        baselineRef,
        comparisonPolicy: resolveComparisonPolicy(config)
      });
    }

    const idempotencyKey = this.#generateIdempotencyKey();
    const runId = `run_${crypto.randomUUID().replace(/-/g, '').slice(0, 24)}`;
    const operation = this.#createOperationDeadline('capture');
    ...
    const baselineId = purpose === 'diff' ? await this.#resolveLatestBaselineId(operation) : null;
    ...
    const comparisonPolicy = resolveComparisonPolicy(config);
    const runResponse = await this.#request(
      'POST',
      `/v1/visual/projects/${this.#projectId}/runs`,
      {
        id: runId,
    ...
    if (comparisonPolicy) {
      assertComparisonPolicyAcknowledged(runResponse?.comparisonPolicy, comparisonPolicy, 'Snap run creation');
    }
    ...
    const outDir = options.outDir ? path.resolve(options.outDir) : await fs.mkdtemp(path.join(os.tmpdir(), 'snapdrift-snap-'));
        ...
        JSON.stringify(
          {
            ...
            screenshots: routes.map((r) => ({
              ...
              path: r.path,
              viewport: r.viewport,
              imagePath: `screenshots/${sanitizeRouteId(r.id)}.png`,
              width: 0,
              height: 0
            }))
          },
    ...
    return {
      ...
      artifacts: { localScreenshots: false, artifactsRoot: undefined },
      selectedRouteIds
    };
  }
  ...
  async diff(options) {
    ...
    let metadataSource;
    try {
      metadataSource = await fs.readFile(currentResultsPath, 'utf-8');
    } catch (error) {
      throw hostedDiffError(
        `could not read capture results metadata at "${currentResultsPath}". ` + `${error instanceof Error ? error.message : String(error)}`
      );
    }
    try {
      ...
      throw hostedDiffError(
        `capture results metadata is not valid JSON; recapture with SnapDrift 0.8.2 or newer. ` + `${error instanceof Error ? error.message : String(error)}`
      );
    }
    const { config } = await loadSnapdriftConfig(options.configPath);
    const diffPlan = validateHostedDiffMetadata(metadata, this.#projectId);
    /** @type {ComparisonPolicy | undefined} */
    let comparisonPolicy;
    if (metadata.comparisonPolicy !== undefined) {
      if (!isComparisonPolicy(metadata.comparisonPolicy)) {
        throw hostedDiffError('capture results metadata has an invalid comparison policy; recapture with the updated SnapDrift client.');
      }
      comparisonPolicy = { ...metadata.comparisonPolicy };
    }

    // Poll until the run reaches a terminal state
    const operation = this.#createOperationDeadline('diff');
    const run = await this.#pollRun(diffPlan.runId, diffPlan.expectedCaptures.length, hostedDiffError, operation);
    if (!run || typeof run !== 'object' || run.id !== diffPlan.runId) {
      throw hostedDiffError(`Snap returned run "${run?.id || 'unknown'}" for requested run "${diffPlan.runId}".`);
    }

    if (comparisonPolicy) {
      assertComparisonPolicyAcknowledged(run.comparisonPolicy, comparisonPolicy, 'Snap run retrieval');
    }

    const summary = this.#mapRunToSummary(run, config, diffPlan, comparisonPolicy);
  ...
  async publishBaseline(options) {
    const resultsPath = options.resultsPath ?? (options.bundleDir ? path.join(path.resolve(options.bundleDir), 'results.json') : undefined);
    ...
    if (resultsPath) {
...

      {
        ...
        const operation = this.#createOperationDeadline('baseline publication');

        if (!Array.isArray(metadata.expectedCaptures) || metadata.expectedCaptures.length === 0) {
          throw completeBaselineError('results metadata has no expected captures; capture again with SnapDrift 0.8.2 or newer.');
        }
        ...
        const run = await this.#pollRun(runId, metadata.expectedCaptures.length, undefined, operation);

        if (run.id && run.id !== runId) {
          throw new Error(`Cannot publish a complete Snap baseline: requested source run ${runId}, but Snap returned ${run.id}.`);
        }
  ...
  async fetchLatestBaseline(_options) {
    const operation = this.#createOperationDeadline('baseline lookup');
    try {
      const baseline = await this.#request('GET', `/v1/visual/projects/${this.#projectId}/baselines/latest`, undefined, undefined, operation);
  ...
  async checkBaselineExists(headSha) {
    const operation = this.#createOperationDeadline('baseline existence check');
    try {
      const baseline = await this.#request(
        'GET',
        `/v1/visual/projects/${this.#projectId}/baselines/latest?sha=${encodeURIComponent(headSha)}`,
        undefined,
        undefined,
        operation
      );
  ...
  async exportBaselines(_options) {
    ...
    const operation = this.#createOperationDeadline('baseline export');
    let archive;
    try {
      archive = await this.#requestBinary('GET', exportPath, operation);
    } catch (error) {
    ...
    if (baselines.length === 0) {
      throw new Error(
        `Snap project "${this.#projectId}" has no accepted baselines to export. ` + `Publish a baseline first (snapdrift baseline with provider: "snap").`
      );
    }
    ...
    if (!routes) {
      throw new Error(`Snap baseline "${baseline.id}" has an unrecognised source manifest format ` + `(expected a "routes" array) and cannot be migrated.`);
    }

    const archivePathByKey = new Map((baseline.objects || []).map((object) => [object.sourceKey, object.archivePath]));

    const sourceLabel = `Snap baseline "${baseline.id}" source manifest`;
    /** @type {Map<string, unknown[]>} */
    const routeIdsByExtension = new Map();
    for (const route of routes) {
      const archivePath = route && route.objectKey ? archivePathByKey.get(route.objectKey) : undefined;
      const extension = typeof archivePath === 'string' ? path.extname(archivePath) || '.png' : '.png';
      const routeIds = routeIdsByExtension.get(extension) || [];
      routeIds.push(route?.routeId);
      routeIdsByExtension.set(extension, routeIds);
    }
    for (const [extension, routeIds] of routeIdsByExtension) {
      assertUniqueRouteIdFilenames(routeIds, sourceLabel, extension);
    }
    ...
    for (const route of routes) {
      ...
      if (!bytes) {
        process.stderr.write(
          `[SnapDrift] Warning: Snap export has no image for route "${route.routeId}" ` + `in baseline ${baseline.id}. Skipping the route.\n`
        );
        continue;
      }
      ...
      const dimensions = readPngDimensions(bytes) ?? (hasDescriptorDims ? { width: descriptor.width, height: descriptor.height } : { width: 1, height: 1 });
      const viewport =
        presetName && VIEWPORT_PRESETS[presetName]
          ? presetName
          : hasDescriptorDims
    ...
    if (screenshots.length === 0) {
      throw new Error(`Snap baseline "${baseline.id}" contained no exportable screenshots — ` + `the export archive had no image bytes for any of its routes.`);
    }

    const captureProfile = safeJsonParse(entries.get(`${baseline.id}/capture_profile.json`)?.toString('utf-8')) ?? {};
   ...
   */
  async #resolveLatestBaselineId(operation = this.#createOperationDeadline('baseline lookup')) {
    try {
      const baseline = await this.#request('GET', `/v1/visual/projects/${this.#projectId}/baselines/latest`, undefined, undefined, operation);
  ...
  /** @param {string} [label] @returns {OperationDeadline} */
  #createOperationDeadline(label = 'Snap operation') {
    return { deadline: this.#nowFn() + MAX_OPERATION_DURATION_MS, label };
  }

  /** @param {OperationDeadline} operation @returns {number} */
  #remainingOperationMs(operation) {
    return operation.deadline - this.#nowFn();
  }

  /**
   * @param {string} requestPath
   * @param {OperationDeadline} operation
   * @returns {SnapUnavailableError}
   */
  #operationDeadlineError(requestPath, operation) {
    return new SnapUnavailableError(
      `Snap ${operation.label} operation deadline exceeded while requesting ${requestPath}; ` + `the ${OPERATION_LIMIT_TEXT} Snap operation limit was reached`
    );
  }
   ...
   * @param {number} timeoutMs
   * @param {OperationDeadline} operation
   * @param {number} requestLimitMs
   * @returns {Error}
   */
  #requestTimeoutError(requestPath, timeoutMs, operation, requestLimitMs) {
    const error = new Error(
      `Snap ${operation.label} request ${requestPath} timed out after ` +
        `${Math.max(0, Math.round(timeoutMs))}ms (request limit ${requestLimitMs}ms; ` +
        `operation limit ${OPERATION_LIMIT_TEXT})`
    );
    error.name = 'SnapTransportTimeoutError';
    return error;
  }

  /** @param {number} attempt @returns {number} */
  #retryDelay(attempt) {
    return Math.min(INITIAL_RETRY_DELAY_MS * RETRY_MULTIPLIER ** (attempt - 1), RETRY_DELAY_CAP_MS);
  }

  /** @param {string} requestPath @param {number} delayMs @param {OperationDeadline} operation @returns {SnapUnavailableError} */
  #sleepTimeoutError(requestPath, delayMs, operation) {
    return new SnapUnavailableError(
      `Snap ${operation.label} wait for ${requestPath} timed out after ${delayMs}ms ` + `before the ${OPERATION_LIMIT_TEXT} operation deadline`
    );
  }

  /**
   * Read and classify an HTTP error response under the current deadline.
   *
   * A received 4xx remains non-retryable even when its diagnostic body cannot
   * be read. Retryable 5xx body failures are returned to the transport loop so
   * they retain the normal availability semantics.
   *
   * @param {any} response
   * @param {string} requestPath
   * @param {AbortController} controller
   * @param {number} attemptDeadline
   * @param {OperationDeadline} operation
   * @param {number} requestLimitMs
   * @returns {Promise<SnapApiError>}
   */
  async #readApiError(response, requestPath, controller, attemptDeadline, operation, requestLimitMs) {
    let errorBody;
    try {
      errorBody = await this.#readResponseBody(response, requestPath, controller, attemptDeadline, operation, requestLimitMs, () => response.text());
    } catch (error) {
      if (response.status < 400 || response.status >= 500) throw error;
      const detail = error instanceof Error ? error.message : String(error);
      const reason =
        error instanceof SnapUnavailableError || error?.name === 'SnapTransportTimeoutError'
          ? `diagnostic body timed out (${detail})`
          : `diagnostic body unavailable (${detail})`;
      return new SnapApiError(response.status, `Snap API ${response.status}: ${reason}`, requestPath);
    }
    ...
    return new SnapApiError(response.status, detail ? `Snap API ${response.status}: ${detail}` : `Snap API ${response.status}`, requestPath);
  }

  /**
   * Read a response body under the current attempt and operation deadlines.
   *
   * @template T
   * @param {any} response
   * @param {string} requestPath
   ...
   * @returns {Promise<T>}
   */
  async #readResponseBody(response, requestPath, controller, attemptDeadline, operation, requestLimitMs, reader) {
    const now = this.#nowFn();
    const attemptRemaining = attemptDeadline - now;
    const operationRemaining = this.#remainingOperationMs(operation);
    if (operationRemaining <= 0) {
      controller.abort();
      cancelResponseBody(response);
      throw this.#operationDeadlineError(requestPath, operation);
    }
    if (attemptRemaining <= 0) {
      controller.abort();
      cancelResponseBody(response);
      throw this.#requestTimeoutError(requestPath, requestLimitMs, operation, requestLimitMs);
    }
    const remaining = Math.min(attemptRemaining, operationRemaining);
    return runWithTimeout(
      ...
      () =>
        this.#remainingOperationMs(operation) <= 0
          ? this.#operationDeadlineError(requestPath, operation)
          : this.#requestTimeoutError(requestPath, remaining, operation, requestLimitMs)
    );
  }

  /**
   * Sleep for a retry or poll interval without exceeding the operation budget.
   *
   ...
   */
  async #sleepWithinOperation(delayMs, operation, requestPath) {
    const remaining = this.#remainingOperationMs(operation);
    if (remaining <= 0) {
      throw this.#operationDeadlineError(requestPath, operation);
    }
    ...
    await runWithTimeout(
      () => this.#sleepFn(delay, controller.signal),
      // Keep a one-millisecond watchdog margin so a sleep timer scheduled by
      // an async injected sleepFn can settle before the watchdog at the same
      // nominal deadline. The signal aborts the default timer on timeout.
      delay + 1,
      () => controller.abort(),
      () => this.#sleepTimeoutError(requestPath, delay, operation)
    );
  }

  /**
   * Shared JSON/binary transport with retries and bounded request bodies.
   *
   * @param {{
   *   method: string,
   *   requestPath: string,
   ...
   *   decode: (response: any, controller: AbortController, attemptDeadline: number, operation: OperationDeadline, requestLimitMs: number) => Promise<any>
   * }} options
   * @returns {Promise<any>}
   */
  async #requestTransport(options) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      const remaining = this.#remainingOperationMs(options.operation);
      if (remaining <= 0) {
        return this.#handleUnavailable(this.#operationDeadlineError(options.requestPath, options.operation));
      }

      const attemptBudget = Math.min(options.requestTimeoutMs, remaining);
      const attemptDeadline = this.#nowFn() + attemptBudget;
      const controller = new AbortController();
      try {
        const response = await runWithTimeout(
          () =>
            this.#fetchFn(`${this.#apiUrl}${options.requestPath}`, {
          ...
          attemptBudget,
          () => controller.abort(),
          () => this.#requestTimeoutError(options.requestPath, attemptBudget, options.operation, options.requestTimeoutMs)
        );

        if (response.ok) {
          return await options.decode(response, controller, attemptDeadline, options.operation, options.requestTimeoutMs);
        }

        // 4xx — non-retryable when the diagnostic body is available. A body
        // read timeout still preserves the received HTTP status, rather than
        // turning an already-classified API response into an availability retry.
        if (response.status >= 400 && response.status < 500) {
          throw await this.#readApiError(response, options.requestPath, controller, attemptDeadline, options.operation, options.requestTimeoutMs);
        }

        // 5xx — retry with backoff unless this is the last attempt. Cancel
        // discarded response bodies so a slow server cannot retain a stream.
        if (attempt < MAX_RETRIES) {
          cancelResponseBody(response);
          await this.#sleepWithinOperation(this.#retryDelay(attempt), options.operation, options.requestPath);
          continue;
        }

        throw await this.#readApiError(response, options.requestPath, controller, attemptDeadline, options.operation, options.requestTimeoutMs);
      } catch (error) {
        if (error instanceof SnapSkipError || error instanceof SnapFallbackError) {
          throw error;
        }
        if (error instanceof SnapApiError && error.status >= 400 && error.status < 500) {
          throw error;
        }
        if (error instanceof SnapApiError && error.status >= 500) {
        ...
        if (this.#remainingOperationMs(options.operation) <= 0) {
          return this.#handleUnavailable(this.#operationDeadlineError(options.requestPath, options.operation));
        }
        ...
        if (attempt < MAX_RETRIES) {
          try {
            ...
            return this.#handleUnavailable(retryError instanceof Error ? retryError : new SnapUnavailableError(String(retryError)));
          }
          continue;
        }

        const detail = error instanceof Error ? error.message : String(error);
        return this.#handleUnavailable(new SnapUnavailableError(`Snap request ${options.requestPath} failed after ${MAX_RETRIES} attempts: ${detail}`));
      } finally {
        controller.abort();
      }
    }
  ...
  /**
   * Make a JSON request with retry and operation deadlines.
   *
   * @param {string} method
   * @param {string} requestPath
   * @param {object} [body]
   ...
   * @returns {Promise<any>}
   */
  async #request(method, requestPath, body, idempotencyKey, operation = this.#createOperationDeadline('Snap request')) {
    return this.#requestTransport({
      ...
      operation,
      requestTimeoutMs: JSON_REQUEST_TIMEOUT_MS,
      decode: async (response, controller, attemptDeadline, requestOperation, requestLimitMs) => {
        const text = await this.#readResponseBody(response, requestPath, controller, attemptDeadline, requestOperation, requestLimitMs, () => response.text());
  ...
  /**
   * Make a binary request with retry and operation deadlines.
   *
   * @param {string} method
   * @param {string} requestPath
   * @param {OperationDeadline} [operation]
   ...
   */
  async #requestBinary(method, requestPath, operation = this.#createOperationDeadline('binary export')) {
    return this.#requestTransport({
      method,
      requestPath,
      operation,
      requestTimeoutMs: BINARY_REQUEST_TIMEOUT_MS,
      decode: async (response, controller, attemptDeadline, requestOperation, requestLimitMs) => {
        const bytes = await this.#readResponseBody(response, requestPath, controller, attemptDeadline, requestOperation, requestLimitMs, () =>
 ...
 * }} options
  ...
  async #captureLocalAndUpload(options) {
    ...
    const baselineId = options.purpose === 'diff' ? await this.#resolveLatestBaselineId(operation) : null;
    ...
    if (options.comparisonPolicy) {
      assertComparisonPolicyAcknowledged(runResponse?.comparisonPolicy, options.comparisonPolicy, 'Snap local-upload run creation');
    }
    ...
    for (const { route, manifestEntry } of uploads) {
      ...
      await this.#request(
        'POST',
        `/v1/visual/captures/${captureId}/local-result`,
        {
    ...
    return { ...localCapture, artifacts: { localScreenshots: true, artifactsRoot: localCapture.screenshotsRoot } };
  }
   ...
   */
  async #pollRun(runId, expectedCaptureCount, errorFactory = (detail) => new Error(detail), operation = this.#createOperationDeadline('run polling')) {
    let incompleteTerminalSignature;
    let stableIncompleteTerminalPolls = 0;
    let incompleteSignature;
    let stableIncompletePolls = 0;

    while (this.#remainingOperationMs(operation) > 0) {
      const requestPath = `/v1/visual/runs/${runId}`;
      const run = await this.#request('GET', requestPath, undefined, undefined, operation);
      ...
      const completeCaptureSet =
        ...
        (captures.length >= expectedCaptureCount && captures.every((capture) => TERMINAL_CAPTURE_STATUSES.has(capture?.status)));

      if (expectedCaptureCount !== undefined && !completeCaptureSet) {
        const signature = JSON.stringify(
          captures
            .map((capture) => [capture?.id, capture?.routeId, capture?.status, capture?.currentObjectKey])
            .sort((left, right) => {
      ...
      if (TERMINAL_RUN_STATUSES.has(run.status)) {
        ...
        if (captures.length < expectedCaptureCount && captures.every((capture) => TERMINAL_CAPTURE_STATUSES.has(capture?.status))) {
          const signature = JSON.stringify(
            captures
              .map((capture) => [capture?.id, capture?.routeId, capture?.status])
              .sort((left, right) => {
                const leftJson = JSON.stringify(left);
                const rightJson = JSON.stringify(right);
                return leftJson < rightJson ? -1 : leftJson > rightJson ? 1 : 0;
      ...
      try {
        await this.#sleepWithinOperation(POLL_INTERVAL_MS, operation, requestPath);
      } catch (error) {
        return this.#handleUnavailable(error instanceof Error ? error : new SnapUnavailableError(String(error)));
      }
    }

    return this.#handleUnavailable(
      new SnapUnavailableError(
        `Snap ${operation.label} run ${runId} did not complete within ${OPERATION_LIMIT_TEXT}; ` + 'the Snap operation deadline was reached while polling'
      )
    );
  }
   ...
   *   expectedByRouteId: Map<string, { routeId: string, routePath: string, viewportDescriptorJson: string, viewport: import('../types/visual-diff-types').VisualViewport }>
   * }} diffPlan
   * @param {ComparisonPolicy} [comparisonPolicy]
   * @returns {import('../types/visual-diff-types').VisualDiffSummary}
   */
  #mapRunToSummary(run, config, diffPlan, comparisonPolicy) {
    const captures = Array.isArray(run.captures) ? run.captures : [];
    ...
    if (run.status === 'error') {
      addError(`run:${run.id}`, undefined, undefined, run.errorDetails?.message || run.errorCode || run.message || 'Snap run ended with status "error".');
    }

    const seenIdentities = new Set();
    const coveredExpectedRoutes = new Set();
    for (const capture of captures) {
      ...
      if (typeof capture.routeId === 'string' && diffPlan.expectedByRouteId.has(capture.routeId)) {
        coveredExpectedRoutes.add(capture.routeId);
      }

      let identity;
      try {
        identity = captureIdentityKey(capture, `source capture "${capture.routeId || capture.id || 'unknown'}"`, hostedDiffError);
      } catch (error) {
        const malformedRouteId = typeof capture.routeId === 'string' && capture.routeId ? capture.routeId : 'unknown';
        const expected = diffPlan.expectedByRouteId.get(malformedRouteId);
        addError(
          malformedRouteId,
          typeof capture.routePath === 'string' ? capture.routePath : expected?.routePath,
          expected?.viewport ?? capture.viewport,
      ...
      if (!expected) {
        addError(capture.routeId, capture.routePath, capture.viewport, `Snap run returned an unexpected route/viewport capture for "${capture.routeId}".`);
        continue;
      }
      if (seenIdentities.has(identity)) {
        addError(expected.routeId, expected.routePath, expected.viewport, `Snap run returned a duplicate route/viewport capture for "${expected.routeId}".`);
        continue;
      }
      seenIdentities.add(identity);

      if (capture.routePath !== expected.routePath) {
        addError(expected.routeId, capture.routePath, expected.viewport, `Snap run returned an unexpected path for "${expected.routeId}".`);
        continue;
      }
      ...
      if (capture.status !== 'new' && capture.status !== 'diffed') {
        addError(
          expected.routeId,
          expected.routePath,
          expected.viewport,
      ...
      if (typeof capture.currentObjectKey !== 'string' || !capture.currentObjectKey) {
        addError(expected.routeId, expected.routePath, expected.viewport, `Snap capture "${expected.routeId}" has no currentObjectKey.`);
        continue;
      }
      ...
      if (capture.baselineObjectKey !== undefined && capture.baselineObjectKey !== null && typeof capture.baselineObjectKey !== 'string') {
        addError(expected.routeId, expected.routePath, expected.viewport, `Snap capture "${expected.routeId}" has an invalid baselineObjectKey.`);
        continue;
      }
      ...
      const comparison = comparisonFromCapture(capture);
      if (comparisonPolicy && !comparison) {
        addError(
          expected.routeId,
          expected.routePath,
          expected.viewport,
          `Snap capture "${expected.routeId}" is missing complete comparison metadata; ` +
            `upgrade the Snap server and diff Lambda before enabling comparisonPolicy v${comparisonPolicy.version}.`
        );
        continue;
      }

      if (comparisonPolicy && comparison && !comparisonMatchesPolicy(comparison, comparisonPolicy)) {
        addError(
          expected.routeId,
          expected.routePath,
          expected.viewport,
          `Snap capture "${expected.routeId}" returned comparison metadata for a different policy than v${comparisonPolicy.version}; ` +
            'upgrade the Snap server and diff Lambda before enabling this comparison policy.'
        );
        continue;
      }

      if (!Number.isFinite(capture.diffPct) || capture.diffPct < 0 || capture.diffPct > 1) {
        addError(expected.routeId, expected.routePath, expected.viewport, `Snap capture "${expected.routeId}" has an invalid diffPct.`);
        continue;
      }

      if (comparisonPolicy && comparison && !hasCompleteComparisonResult(capture, comparison, comparisonPolicy.threshold)) {
        addError(
          expected.routeId,
          expected.routePath,
          expected.viewport,
          `Snap capture "${expected.routeId}" has incomplete or inconsistent comparison statistics.`
        );
        continue;
      }

      if (comparison && (!Number.isInteger(capture.diffPixels) || capture.diffPixels < 0 || capture.diffPixels > comparison.totalPixels)) {
        addError(expected.routeId, expected.routePath, expected.viewport, `Snap capture "${expected.routeId}" has invalid comparison pixel statistics.`);
        continue;
      }

      const legacyWidth = capture.width || 0;
      const legacyHeight = capture.height || 0;
      const dimensionsChanged = comparison?.dimensionsChanged === true;
      if (dimensionsChanged || capture.diffPct > comparisonThreshold) {
        ...
        changed.push({
          ...
          width: comparison?.canvas.width ?? legacyWidth,
          height: comparison?.canvas.height ?? legacyHeight,
          differentPixels: capture.diffPixels || 0,
          totalPixels: comparison?.totalPixels ?? legacyWidth * legacyHeight,
    ...
    for (const expected of diffPlan.expectedByRouteId.values()) {
      if (coveredExpectedRoutes.has(expected.routeId)) continue;
      missingInCurrent++;
      missing.push({
        id: expected.routeId,
...
/**
 * Error raised when an updated client opts into a comparison policy that the
 * connected Snap server does not acknowledge.
 */
export class SnapUpgradeRequiredError extends Error {
  /** @type {'comparison_policy_unsupported'} */
  code = 'comparison_policy_unsupported';

  /** @param {string} message */
  constructor(message) {
    super(message);
    this.name = 'SnapUpgradeRequiredError';
  }
}
