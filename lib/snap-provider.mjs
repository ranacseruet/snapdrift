...
export class SnapProvider {
  ...
  async capture(options) {
    ...
    // Resolve the latest accepted baseline so the Snap backend can attach it to
    // this run's captures. Without a baselineId the server has nothing to diff
    // against and short-circuits every capture to "diffed" with no comparison
    // data, leaving the dashboard baseline + diff panes empty.
