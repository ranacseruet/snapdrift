...
export class SnapProvider {
  ...
  #operationDeadlineError(requestPath, operation) {
    return new SnapUnavailableError(
      ...
      'the 10 minute Snap operation limit was reached'
    );
  }
  ...
  async #readResponseBody(response, requestPath, controller, attemptDeadline, operation, requestLimitMs, reader) {
    const remaining = Math.min(
      attemptDeadline - this.#nowFn(),
      this.#remainingOperationMs(operation)
    );
    if (remaining <= 0) {
      controller.abort();
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (attempt < MAX_RETRIES) {
          cancelResponseBody(response);
          await this.#sleepWithinOperation(
            Math.min(INITIAL_RETRY_DELAY_MS * (RETRY_MULTIPLIER ** (attempt - 1)), RETRY_DELAY_CAP_MS),
            options.operation,
            options.requestPath
          );
          continue;
        ...
        const errorBody = await this.#readResponseBody(
          response,
          options.requestPath,
          controller,
          attemptDeadline,
  ...
  async #pollRun(
    ...
    return this.#handleUnavailable(new SnapUnavailableError(
      `Snap ${operation.label} run ${runId} did not complete within 10 minutes; ` +
