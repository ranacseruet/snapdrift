...
import { makeMarkdown } from '@snapdrift/adapter-report-md';
...
function getPackageVersion() {
  try {
    ...
    const pkg = JSON.parse(fsSync.readFileSync(new URL(pkgUrl), 'utf-8'));
...
export class SnapProvider {
  ...
  #handleUnavailable(error) {
    ...
    if (mode === 'warn-and-skip') {
      ...
      throw error; // Signal to caller that the check was skipped
    }
