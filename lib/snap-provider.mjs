...
function isComparisonMetadata(value) {
  ...
  if (comparison.mode === 'coordinate-fallback') {
    return (
      ...
      ['width-mismatch', 'alignment-limit', 'ambiguous', 'verification-failed', 'ignore-regions', 'alignment-unavailable'].includes(comparison.fallbackReason) &&
      canvas.width === Math.max(baseline.width, current.width) &&
      canvas.height === Math.max(baseline.height, current.height) &&
      comparison.totalPixels <= canvas.width * canvas.height
    );
  ...
  for (const segment of comparison.rowMapping) {
    if (!segment || !['matched', 'changed', 'inserted', 'deleted'].includes(segment.kind) || !Number.isInteger(segment.outputStart) || segment.outputStart !== outputStart || !Number.isInteger(segment.length) || segment.length <= 0) {
      return false;
    }
