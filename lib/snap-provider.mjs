...
function isComparisonPolicy(value) {
  ...
  return policy.version === 1 && typeof policy.threshold === 'number' && Number.isFinite(policy.threshold) && policy.threshold >= 0 && policy.threshold <= 1;
}
...
function assertComparisonPolicyAcknowledged(actual, expected, context) {
  if (!isComparisonPolicy(actual) || actual.threshold !== expected.threshold) {
    throw new SnapUpgradeRequiredError(
      `${context} did not acknowledge comparisonPolicy v1 with threshold ${expected.threshold}. ` +
        'Upgrade the Snap server before enabling unequal-dimension comparisons.'
    );
  }
}
      ...
      if (comparisonPolicy && !comparison) {
        addError(
          ...
          `Snap capture "${expected.routeId}" is missing complete comparison metadata; ` +
            'upgrade the Snap server and diff Lambda before enabling comparisonPolicy v1.'
        );
        continue;
      }
