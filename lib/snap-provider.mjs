...
function normalizeViewportDescriptor(value, context) {
  let descriptor;
  try {
    descriptor = typeof value === 'string' ? JSON.parse(value) : value;
  } catch {
    throw completeBaselineError(`${context} has malformed viewportDescriptorJson.`);
  }
  if (!descriptor || typeof descriptor !== 'object' || Array.isArray(descriptor) ||
      !Number.isFinite(descriptor.width) || !Number.isFinite(descriptor.height)) {
    throw completeBaselineError(`${context} has an invalid viewport descriptor.`);
  }
...
/** @param {{ routeId?: unknown, viewportDescriptorJson?: unknown }} capture @param {string} context */
function captureIdentityKey(capture, context) {
  if (typeof capture.routeId !== 'string' || !capture.routeId) {
    throw completeBaselineError(`${context} has no routeId.`);
  }
  return `${capture.routeId}\u0000${normalizeViewportDescriptor(capture.viewportDescriptorJson, context)}`;
}

/** @param {unknown} value @param {string} field @returns {Set<string>} */
function uniqueRouteIdSet(value, field) {
  if (!Array.isArray(value) || value.some((id) => typeof id !== 'string' || !id)) {
    throw completeBaselineError(`results metadata has no valid ${field}; capture again with SnapDrift 0.8.2 or newer.`);
  }
  const ids = new Set(value);
  if (ids.size !== value.length) {
    throw completeBaselineError(`results metadata ${field} contains duplicate route ids.`);
  }
  return ids;
}
...
function resolveHostedBaselineCiRef() {
  if (process.env.GITHUB_ACTIONS !== 'true') {
    throw new Error(
      'Hosted Snap baselines may only be published by GitHub Actions on the project default branch. ' +
      'Use provider "local" for a baseline created from a developer machine.'
    );
  }
  const refBranch = process.env.GITHUB_REF_NAME;
  const refSha = process.env.GITHUB_SHA;
  const publicationWorkflowRef = process.env.GITHUB_WORKFLOW_REF;
  const publicationSequence = Number(process.env.GITHUB_RUN_NUMBER);
  if (
    !refBranch ||
    !refSha ||
    ...
    throw new Error(
      'Hosted Snap baseline publication requires GITHUB_REF_NAME, a resolved 40-character ' +
      'GITHUB_SHA, GITHUB_WORKFLOW_REF, and a positive GITHUB_RUN_NUMBER.'
    );
  }
...
export class SnapProvider {
  ...
  async capture(options) {
    ...
    const baselineRef = purpose === 'baseline' ? resolveHostedBaselineCiRef() : undefined;
   ...
   */
  async #pollRun(runId, expectedCaptureCount) {
