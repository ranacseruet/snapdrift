...
function validateHostedDiffMetadata(metadata, projectId) {
  ...
  if (metadata.purpose !== 'diff') {
    throw hostedDiffError(
      `capture results have purpose "${metadata.purpose || 'unknown'}", not "diff"; ` +
      'recapture with SnapDrift 0.8.2 or newer.'
    );
  }
  ...
  for (const [index, expected] of metadata.expectedCaptures.entries()) {
    ...
    const viewportDescriptorJson = normalizeViewportDescriptor(
...
function validateCompleteBaselineRun(metadata, run, projectId) {
  ...
  for (const expected of metadata.expectedCaptures) {
    ...
    expectedByIdentity.set(
      captureIdentityKey(expected, `expected capture "${expected.routeId}"`, completeBaselineError),
      expected
    );
  }
  ...
  const routes = run.captures.map((capture) => {
    ...
    const identity = captureIdentityKey(
      capture,
      `source capture "${capture.routeId || capture.id || 'unknown'}"`,
      completeBaselineError
    );
...
export class SnapProvider {
  ...
  constructor(snapConfig, options = {}) {
    ...
    this.#sleepFn = options.sleepFn ?? ((ms, signal) => new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
  ...
  async diff(options) {
    ...
    try {
      ...
      throw hostedDiffError(
        `could not read capture results metadata at "${currentResultsPath}". ` +
   ...
   *
   * @param {Parameters<typeof buildReportCommentBody>[0]} summary
   * @param {import('../types/visual-diff-types').ProviderCommentMeta} [meta]
   * @returns {string}
   */
  buildCommentBody(summary, meta = {}) {
    const dashboardUrl = /** @type {{ dashboardUrl?: string }} */ (summary).dashboardUrl || meta.dashboardUrl;
  ...
  async checkBaselineExists(headSha) {
    ...
    try {
      const baseline = await this.#request('GET', `/v1/visual/projects/${this.#projectId}/baselines/latest?sha=${encodeURIComponent(headSha)}`, undefined, undefined, operation);
      return baseline;
    } catch (error) {
      if (error instanceof SnapApiError && error.status === 404) {
        return null;
  ...
  #operationDeadlineError(requestPath, operation) {
    return new SnapUnavailableError(
      `Snap ${operation.label} operation deadline exceeded while requesting ${requestPath}; ` +
      `the ${OPERATION_LIMIT_TEXT} Snap operation limit was reached`
    );
  }
  ...
  #sleepTimeoutError(requestPath, delayMs, operation) {
    return new SnapUnavailableError(
      `Snap ${operation.label} wait for ${requestPath} timed out after ${delayMs}ms ` +
      `before the ${OPERATION_LIMIT_TEXT} operation deadline`
    );
  }
  ...
  async #readApiError(response, requestPath, controller, attemptDeadline, operation, requestLimitMs) {
    let errorBody;
    try {
      errorBody = await this.#readResponseBody(
        response,
        requestPath,
        controller,
        attemptDeadline,
      ...
      const reason = error instanceof SnapUnavailableError || error?.name === 'SnapTransportTimeoutError'
        ? `diagnostic body timed out (${detail})`
        : `diagnostic body unavailable (${detail})`;
      return new SnapApiError(
        response.status,
        `Snap API ${response.status}: ${reason}`,
        requestPath
      );
    }
    ...
    return new SnapApiError(
      response.status,
      detail ? `Snap API ${response.status}: ${detail}` : `Snap API ${response.status}`,
      requestPath
    );
  }
      ...
      try {
        ...
        if (attempt < MAX_RETRIES) {
          try {
            ...
            return this.#handleUnavailable(retryError instanceof Error
              ? retryError
              : new SnapUnavailableError(String(retryError)));
          }
          continue;
        }
        ...
        return this.#handleUnavailable(
          new SnapUnavailableError(
            `Snap request ${options.requestPath} failed after ${MAX_RETRIES} attempts: ${detail}`
          )
        );
      } finally {
        controller.abort();
  ...
  async #requestBinary(method, requestPath, operation = this.#createOperationDeadline('binary export')) {
    return this.#requestTransport({
      ...
      decode: async (response, controller, attemptDeadline, requestOperation, requestLimitMs) => {
        const bytes = await this.#readResponseBody(
          response,
          requestPath,
          controller,
          attemptDeadline,
  ...
  async #captureLocalAndUpload(options) {
    ...
    await this.#request('POST', `/v1/visual/projects/${this.#projectId}/runs`, {
      ...
      ...options.runContext
    }, this.#generateIdempotencyKey(), operation);
  ...
  async #pollRun(
    ...
    errorFactory = (detail) => new Error(detail),
    operation = this.#createOperationDeadline('run polling')
  ) {
    ...
    while (this.#remainingOperationMs(operation) > 0) {
      ...
      try {
        ...
        return this.#handleUnavailable(error instanceof Error
          ? error
          : new SnapUnavailableError(String(error)));
      }
    }

    return this.#handleUnavailable(new SnapUnavailableError(
      `Snap ${operation.label} run ${runId} did not complete within ${OPERATION_LIMIT_TEXT}; ` +
      'the Snap operation deadline was reached while polling'
    ));
  }
  ...
  #mapRunToSummary(run, config, diffPlan) {
    ...
    for (const capture of captures) {
      ...
      try {
        ...
        const malformedRouteId = typeof capture.routeId === 'string' && capture.routeId
          ? capture.routeId
          : 'unknown';
