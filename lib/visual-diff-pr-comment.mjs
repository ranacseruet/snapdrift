...
export const LEGACY_PR_COMMENT_MARKER = '<!-- pr-visual-diff-summary -->';
export const PR_COMMENT_MARKERS = [PR_COMMENT_MARKER, LEGACY_PR_COMMENT_MARKER];
 ...
 * @param {{ artifactName?: string, runUrl?: string, repoUrl?: string, iconUrl?: string }} [meta]
 * @returns {string}
 */
export function buildPrCommentBody(summary, meta = {}) {
  ...
  const repoUrl = resolveUrl(meta.repoUrl, DEFAULT_SNAPDRIFT_REPO_URL);
  ...
  const lines = [
    ...
    `| Stable captures | ${summary.matchedScreenshots || 0} |`,
  ...
  lines.push('');
  lines.push(`<div align="right"><sub>Powered by <a href="${repoUrl}">SnapDrift</a></sub></div>`);
