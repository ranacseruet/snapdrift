 ...
 *
 * @param {JsonObject} workflow - Parsed YAML workflow
 * @returns {{ step: JsonObject, jobName: string, stepIndex: number } | null}
 */
 ...
 *
 * @param {JsonObject} step - The workflow step using the Snap action
...
function translateStepToConfig(step) {
  ...
  if (baselineTag) {
   ...
   // Snap provider config (if present in inputs). A hosted Snap provider requires
   // an API-key source; only enable provider: "snap" when one is present. When a
   // project ID exists without an API-key source, keep a valid (local) config and
   // emit a clear migration warning rather than emitting provider: "snap" with no
   // API-key source, which later fails validation.
   ...
   if (snapApiKeyEnv) {
     ...
     snap.apiKeyEnv = snapApiKeyEnv;
     if (snapProjectId) snap.projectId = snapProjectId;
   } else if (snapProjectId) {
     warnings.push({
       field: 'snap.projectId',
       originalValue: snapProjectId,
       message: `Snap project ID "${snapProjectId}" is present but no API-key source (snap-api-key-env) was configured. SnapDrift keeps the local provider; add a Snap API key source to enable hosted baselines.`,
       severity: 'warning'
     });
   }
...
export async function runInitFromAction(actionYamlPath) {
  ...
  const workflow = /** @type {JsonObject} */ (parsedWorkflow);
