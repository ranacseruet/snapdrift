...
import {
  SnapProvider,
  SnapApiError,
  SnapUpgradeRequiredError,
  SnapUnavailableError,
  SnapFallbackError,
  SnapSkipError,
  isLocalBaseUrl
} from './snap-provider.mjs';
...
export class LocalProvider {
  ...
  async capture(options) {
    const result = await runBaselineCapture(options);
    return { ...result, artifacts: { localScreenshots: true, artifactsRoot: result.screenshotsRoot } };
  }
   ...
   * Guard with providerSupports(providerName, 'fetchLatestBaseline') before calling.
  ...
  async fetchLatestBaseline(_options) {
    throw new Error(
      'LocalProvider.fetchLatestBaseline is not yet implemented. ' + 'Use the pr-diff action for baseline resolution, or wait for a future release.'
    );
  }

  /**
   * Build a PR comment body from a diff summary.
   * LocalProvider omits the dashboard link (no dashboard exists).
   *
  ...
  buildCommentBody(summary, meta = {}) {
    return buildReportCommentBody(summary, meta);
  }
}

export const PROVIDER_CAPABILITIES = Object.freeze({
  local: Object.freeze({ capture: true, diff: true, publishBaseline: true, fetchLatestBaseline: false, buildCommentBody: true }),
  snap: Object.freeze({ capture: true, diff: true, publishBaseline: true, fetchLatestBaseline: true, buildCommentBody: true })
});
 ...
 * @returns {boolean}
 */
export function providerSupports(providerName, capability) {
  return Object.hasOwn(PROVIDER_CAPABILITIES, providerName) &&
    Object.hasOwn(PROVIDER_CAPABILITIES[providerName], capability) &&
    PROVIDER_CAPABILITIES[providerName][capability] === true;
}

/** @type {{ local: (config?: VisualRegressionConfig) => LocalProvider, snap: (config: VisualRegressionConfig) => SnapProvider }} */
...
/**
 * @overload
 * @param {'snap'} providerName
 * @param {VisualRegressionConfig} config
 * @returns {SnapProvider}
 */
/**
 * @overload
 * @param {'local'} providerName
 * @param {VisualRegressionConfig} [config]
...
export function createProvider(providerName, config) {
  ...
  if (!factory) {
    throw new Error(`Unknown SnapDrift provider: "${providerName}". ` + `Available providers: ${Object.keys(PROVIDER_FACTORIES).join(', ')}.`);
  }
  return factory(config);
}

export { SnapProvider, SnapApiError, SnapUpgradeRequiredError, SnapUnavailableError, SnapFallbackError, SnapSkipError, isLocalBaseUrl };
