...
const baselineResultsPath = readFirstDefinedEnv(['SNAPDRIFT_BASELINE_RESULTS_PATH', 'QA_VISUAL_BASELINE_RESULTS_PATH']);
const baselineManifestPath = readFirstDefinedEnv(['SNAPDRIFT_BASELINE_MANIFEST_PATH', 'QA_VISUAL_BASELINE_MANIFEST_PATH']);
const currentResultsPath = readFirstDefinedEnv(['SNAPDRIFT_CURRENT_RESULTS_PATH', 'QA_VISUAL_CURRENT_RESULTS_PATH']);
const currentManifestPath = readFirstDefinedEnv(['SNAPDRIFT_CURRENT_MANIFEST_PATH', 'QA_VISUAL_CURRENT_MANIFEST_PATH']);
const baselineRunDir = path.resolve(readFirstDefinedEnv(['SNAPDRIFT_BASELINE_RUN_DIR', 'QA_VISUAL_BASELINE_RUN_DIR']) || 'baseline');
const currentRunDirValue = readFirstDefinedEnv(['SNAPDRIFT_CURRENT_RUN_DIR', 'QA_VISUAL_CURRENT_RUN_DIR']);
const currentRunDir = currentRunDirValue ? path.resolve(currentRunDirValue) : '';
const outDir = path.resolve(
  readFirstDefinedEnv(['SNAPDRIFT_DRIFT_OUT_DIR', 'QA_VISUAL_DIFF_OUT_DIR']) || path.join('qa-artifacts', 'snapdrift', 'drift', 'current')
);
const summaryPath = path.resolve(
  readFirstDefinedEnv(['SNAPDRIFT_SUMMARY_PATH', 'QA_VISUAL_DIFF_SUMMARY_PATH']) || path.join(outDir, 'summary.json')
);
const markdownPath = path.resolve(
  readFirstDefinedEnv(['SNAPDRIFT_SUMMARY_MARKDOWN_PATH', 'QA_VISUAL_DIFF_SUMMARY_MARKDOWN']) || path.join(outDir, 'summary.md')
);
