...
/**
 * @param {unknown} outageError
 * @param {'capture' | 'diff'} phase
 * @param {string | undefined} lookupMessage
 * @returns {Error}
 */
function createBaselineLookupFallbackError(outageError, phase, lookupMessage) {
  const lookupDetail = typeof lookupMessage === 'string' && lookupMessage.trim().length > 0
    ? ` ${lookupMessage.trim()}`
    : '';
  const outageDetail = outageError instanceof Error && outageError.message
    ? ` Snap fallback reason: ${outageError.message}`
    : '';
  return new Error(
    `Cannot fall back to a local ${phase} because the GitHub baseline lookup failed.${lookupDetail}${outageDetail}`,
    { cause: outageError }
  );
}
...
export function hasLocalScreenshots(providerName, config) {
  return describeCaptureArtifacts(providerName, undefined, config).localScreenshots;
}

/**
 * Prefer provider-returned artifacts; infer capabilities only for legacy results.
 *
 * @param {string} providerName
 * @param {Partial<CaptureResult>} [result]
 * @param {VisualRegressionConfig} [config]
 * @returns {{ localScreenshots: boolean, artifactsRoot?: string }}
 */
export function describeCaptureArtifacts(providerName, result, config) {
  if (result?.artifacts) return result.artifacts;
  const localScreenshots = providerName !== 'snap' || isLocalBaseUrl(config?.baseUrl);
  return { localScreenshots, artifactsRoot: localScreenshots ? result?.screenshotsRoot : undefined };
}
 ...
 *   baselineResolutionStatus?: string,
 *   baselineResolutionMessage?: string
 * }} options
 * @returns {Promise<{
 ...
 *   artifacts: ReturnType<typeof describeCaptureArtifacts>
 * }>}
 */
export async function captureWithPolicy(options) {
  ...
  try {
    ...
    const artifacts = describeCaptureArtifacts(providerName, result, options.config);
    return {
      outcome: 'captured',
      providerName,
      result,
      artifacts,
      localScreenshots: artifacts.localScreenshots
    };
  } catch (error) {
    if (error instanceof SnapSkipError) {
      onSkip(error);
      return { outcome: 'skipped', providerName, localScreenshots: false, artifacts: { localScreenshots: false } };
    }
    if (error instanceof SnapFallbackError) {
      onFallback(error);
      if (options.baselineResolutionStatus === 'error') {
        throw createBaselineLookupFallbackError(error, 'capture', options.baselineResolutionMessage);
      }
      ...
      const artifacts = describeCaptureArtifacts('local', result, options.config);
      return { outcome: 'captured', providerName: 'local', result, localScreenshots: artifacts.localScreenshots, artifacts };
    }
    throw error;
  }
}
 ...
 *
 * Capability precedence: legacy `localScreenshots` overrides `artifacts`, then
 * `captureResult.artifacts`, then provider/config inference. Without any of those
 * inputs (including config), preserve the legacy local-screenshots default.
 *
 * @param {{
 *   provider: VisualProvider,
 *   providerName?: string,
...
export async function diffWithPolicy(options) {
  const providerName = options.providerName || 'local';
  /** @type {Partial<CaptureResult>} */
  const captureResult = options.captureResult || { screenshotsRoot: options.diffOptions.currentRunDir };
  const described = options.artifacts || (captureResult.artifacts || options.config
    ? describeCaptureArtifacts(providerName, captureResult, options.config)
    : { localScreenshots: true, artifactsRoot: captureResult.screenshotsRoot });
  let artifacts = {
    ...described,
    localScreenshots: options.localScreenshots ?? described.localScreenshots
  };
  if (!artifacts.localScreenshots) artifacts.artifactsRoot = undefined;
  const createLocalProvider = options.createLocalProvider || (() => createProvider('local'));
  const onSkip = options.onSkip || noop;
  const onFallback = options.onFallback || noop;
  ...
  try {
    ...
    return { outcome: 'diffed', providerName, result, artifacts, localScreenshots: artifacts.localScreenshots };
  } catch (error) {
    if (error instanceof SnapSkipError) {
      onSkip(error);
      return { outcome: 'skipped', providerName, artifacts, localScreenshots: artifacts.localScreenshots };
    }
    if (!(error instanceof SnapFallbackError)) {
      throw error;
    }
    ...
    if (!baselineAvailable) {
      ...
      if (options.baselineResolutionStatus === 'error') {
        throw createBaselineLookupFallbackError(error, 'diff', options.baselineResolutionMessage);
      }
      return { outcome: 'baseline-unavailable', providerName: 'local', artifacts, localScreenshots: artifacts.localScreenshots };
    }
    ...
    if (!artifacts.localScreenshots) {
      ...
      artifacts = describeCaptureArtifacts('local', recapture, options.config);
      diffOptions = {
        ...options.diffOptions,
    ...
    return { outcome: 'diffed', providerName: 'local', result, recapture, artifacts, localScreenshots: artifacts.localScreenshots };
  }
}
