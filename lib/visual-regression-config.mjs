...
export const LEGACY_CONFIG_PATH = path.resolve('.github', 'visual-regression.json');
 ...
 * @returns {VisualRegressionConfig}
 */
export function validateVisualRegressionConfig(value, sourceLabel = 'inline config') {
  /** @type {string[]} */
  const errors = [];
  ...
  if (!Array.isArray(candidate.routes) || candidate.routes.length === 0) {
    ...
    for (const [index, route] of candidate.routes.entries()) {
      ...
      if (!isNonEmptyString(route.viewport) || !VALID_VIEWPORT_PRESETS.has(route.viewport)) {
        errors.push(`${routeLabel}.viewport must be one of: ${[...VALID_VIEWPORT_PRESETS].join(', ')}.`);
      }
  ...
  return /** @type {VisualRegressionConfig} */ (/** @type {unknown} */ (value));
}
 ...
 */
export async function loadVisualRegressionConfig(configPath = readFirstDefinedEnv(['SNAPDRIFT_CONFIG_PATH', 'QA_VISUAL_CONFIG_PATH'])) {
  ...
  if (configPath) {
    ...
    try {
      ...
      resolvedConfigPath = DEFAULT_CONFIG_PATH;
    } catch {
  ...
  return {
    config: validateVisualRegressionConfig(parsed, resolvedConfigPath),
