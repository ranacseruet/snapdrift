 ...
 *
 * Values may be given inline as `--flag=value` (so values beginning with a hyphen,
 * e.g. `--routes=-mobile`, work) or after the `--` end-of-options marker. Unknown
 * flags and missing option values are rejected with a non-zero error; invalid
 * *values* for --to/--from remain silently ignored (validated downstream).
 *
...
export function parseArgs(argv) {
  ...
  let fromSnapAction;
   ...
   // (their validation happens downstream). Values that begin with a hyphen (e.g.
   // a route id like `-mobile`) may be supplied inline as `--flag=value` or bare.
   ...
   const BOOLEAN_FLAGS = new Set(['--open', '--accept-cross-engine']);

   /**
    * Assign a parsed value (from an inline `--flag=value` or a following token)
    * to the option named by `flag`.
    *
    * @param {string} flag
    * @param {string} value
    */
   function assignFlag(flag, value) {
     if (flag === '--routes') {
     ...
     } else if (flag === '--current-dir') {
       currentDir = path.resolve(value);
     } else if (flag === '--diff-dir') {
       diffDir = path.resolve(value);
     } else if (flag === '--to') {
       if (value === 'snap' || value === 'local') to = value;
     } else if (flag === '--from') {
       if (value === 'snap') from = value;
     } else if (flag === '--from-snap-action') {
   ...
   for (let i = 1; i < args.length; i++) {
     ...
     // End-of-options: after `--`, tokens are values/positionals, never flags.
     if (endOfOptions) {
       if (VALUE_FLAGS.has(raw)) {
         const value = args[i + 1];
         if (value === undefined) {
           throw new Error(`Missing value for flag ${raw}`);
         }
         assignFlag(raw, value);
         i++; // consume the following token so it is not reprocessed as a flag
       }
       continue;
     }

     // Inline `--flag=value` form (also handles values that begin with a hyphen).
     ...
     if (raw.startsWith('--') && eq > 0) {
       ...
       assignFlag(flag, raw.slice(eq + 1));
       continue;
     }
     ...
     if (BOOLEAN_FLAGS.has(raw)) {
       if (raw === '--open') {
         open = true;
       } else if (raw === '--accept-cross-engine') {
         acceptCrossEngine = true;
       }
     } else if (VALUE_FLAGS.has(raw)) {
       const value = args[i + 1];
       // A hyphenated following token (e.g. a route id like `-mobile`) is a
       // legitimate value; only a missing token, the `--` end-of-options marker,
       // or a following *recognized flag* means the value is absent.
       if (value === undefined || value === '--' || VALUE_FLAGS.has(value) || BOOLEAN_FLAGS.has(value)) {
         throw new Error(`Missing value for flag ${raw}`);
       }
       assignFlag(raw, value);
       i++; // consume the following token so it is not reprocessed as a flag
...
export function printSummary(summary) {
  ...
  if (summary.captureCompatibility?.status === 'unverified' && summary.message) {
    process.stdout.write(`   Note: ${summary.message}\n`);
  }
 ...
 * so metacharacters in a local path cannot execute commands. On Windows, `start`
 * is a cmd.exe built-in rather than an executable, so spawnSync (which does not
 * use a shell by default) returns ENOENT; open the registered handler through
 * rundll32, a real executable, keeping the path as a separate argument.
 *
 * @param {string} filePath
...
export async function runDiffCommand(opts) {
  ...
  const diffOptions = /** @type {import('../types/visual-diff-types').ProviderDiffOptions & { diffImagesDir: string }} */ ({
    configPath: opts.configPath,
    routeIds: opts.routes.length > 0 ? opts.routes : undefined,
    baselineResultsPath,
    ...
    diffImagesDir: path.join(opts.diffDir, 'diffs')
  });
