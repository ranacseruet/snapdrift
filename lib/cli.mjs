...
import { SnapFallbackError, SnapSkipError } from './snap-provider.mjs';
...
export async function runCaptureCommand(opts) {
  ...
  try {
    const { selectedRouteIds } = await provider.capture({
    ...
    if (error instanceof SnapSkipError) {
      process.stdout.write('Capture skipped (Snap unavailable, warn-and-skip mode).\n');
      return;
    }
    if (error instanceof SnapFallbackError) {
      process.stdout.write('Falling back to local provider for capture...\n');
      const localProvider = createProvider('local');
      const { selectedRouteIds } = await localProvider.capture({
        configPath: opts.configPath,
        routeIds: opts.routes.length > 0 ? opts.routes : undefined,
        outDir: opts.baselineDir
      });
...
export async function runBaselineCommand(opts) {
  ...
  /** Capture locally so `fallback-local` still leaves the user with a baseline. */
  async function captureLocalFallback() {
    process.stdout.write('Falling back to local provider for capture...\n');
    const localProvider = createProvider('local');
    const { selectedRouteIds } = await localProvider.capture({
      configPath: opts.configPath,
    ...
    process.stdout.write(
      `Captured ${selectedRouteIds.length} route(s) to ${opts.baselineDir} (local fallback — nothing published to Snap)\n`
    );
  }

  /**
   * Apply the configured `onUnavailable` behaviour. Snap can go down during
   * *either* phase — the capture submission or the publish that follows it — and
   * the user's choice must hold for both, otherwise `warn-and-skip` still exits 1
   * and `fallback-local` still leaves them with no baseline.
   *
   * @param {unknown} error
   * @param {string} skipMessage
   * @returns {Promise<boolean>} true when the error was absorbed
   */
  async function handleUnavailable(error, skipMessage) {
    if (error instanceof SnapSkipError) {
      process.stdout.write(skipMessage);
      return true;
    }
    if (error instanceof SnapFallbackError) {
      await captureLocalFallback();
      return true;
    }
    return false;
  }

  /** @type {{ resultsPath?: string, selectedRouteIds: string[] }} */
  let captured;
  try {
    ...
    if (await handleUnavailable(error, 'Baseline skipped (Snap unavailable, warn-and-skip mode).\n')) {
      return;
    }
    throw error;
  }
  ...
  if (providerName !== 'snap') {
    process.stdout.write(`Captured ${captured.selectedRouteIds.length} route(s) to ${opts.baselineDir}\n`);
    return;
  }

  process.stdout.write(
    `Captured ${captured.selectedRouteIds.length} route(s); waiting for Snap to finish rendering ...\n`
  );
  try {
    await provider.publishBaseline({ resultsPath: captured.resultsPath });
  } catch (error) {
    if (
      await handleUnavailable(
        error,
        'Baseline publish skipped (Snap unavailable, warn-and-skip mode).\n'
      )
    ) {
      return;
    }
...
export async function runDiffCommand(opts) {
  ...
  try {
    currentCapture = await provider.capture({
      configPath: opts.configPath,
      routeIds: opts.routes.length > 0 ? opts.routes : undefined,
    ...
    if (error instanceof SnapFallbackError) {
      process.stdout.write('Falling back to local provider for capture...\n');
      const localProvider = createProvider('local');
      currentCapture = await localProvider.capture({
        configPath: opts.configPath,
        routeIds: opts.routes.length > 0 ? opts.routes : undefined,
        outDir: opts.currentDir
      });
  ...
  const diffProvider = usedLocalFallback ? createProvider('local') : provider;
  let diffResult;
  try {
    diffResult = await diffProvider.diff({
      configPath: opts.configPath,
      routeIds: opts.routes.length > 0 ? opts.routes : undefined,
      baselineResultsPath,
