...
export async function runBaselineCommand(opts) {
  ...
  try {
    ...
    // onUnavailable: warn-and-skip — Snap is down and the user asked not to fail
    // the run for it. Matches runDiffCommand: warn and exit 0, no baseline.
    if (error instanceof SnapSkipError) {
      process.stdout.write('Baseline skipped (Snap unavailable, warn-and-skip mode).\n');
      return;
    }
    // onUnavailable: fallback-local — the baseline becomes a local one, so there
    // is nothing to publish to Snap afterwards.
    if (error instanceof SnapFallbackError) {
      process.stdout.write('Falling back to local provider for capture...\n');
      const localProvider = createProvider('local');
      const { selectedRouteIds } = await localProvider.capture({
