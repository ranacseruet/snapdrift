...
import { SnapFallbackError } from './snap-provider.mjs';
...
export async function runDiffCommand(opts) {
  ...
  try {
    ...
    if (!usedLocalFallback && error instanceof SnapFallbackError) {
      process.stdout.write('Falling back to local provider for diff...\n');
      const localProvider = createProvider('local');
      diffResult = await localProvider.diff({
        configPath: opts.configPath,
        routeIds: opts.routes.length > 0 ? opts.routes : undefined,
        baselineResultsPath,
        baselineManifestPath,
