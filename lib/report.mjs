...
export async function generateHtmlReport(summary, options = {}) {
  ...
  if (summary.changed.length === 0) {
    ...
    const rows = await Promise.all(summary.changed.map(async (item) => {
      ...
      if (baselineRunDir) {
        const b64 = await readFileAsBase64(path.resolve(baselineRunDir, item.baselineImagePath));
        baselineImgHtml = imgTag(b64, `Baseline: ${item.id}`);
      }
      if (currentRunDir) {
        const b64 = await readFileAsBase64(path.resolve(currentRunDir, item.currentImagePath));
        currentImgHtml = imgTag(b64, `Current: ${item.id}`);
      }
