...
- name: SnapDrift Baseline
  uses: ranacseruet/snapdrift@v0.14.0
  with:
    mode: baseline
...
- Unequal dimensions use deprecated top-left union comparison policy v1 by default for compatibility; new configurations should opt into policy v2 with `{ "version": 2, "threshold": number }` for bounded vertical row alignment of inserted/deleted content
