...
/**
 * A semantic palette diff image is only generated by the local adapter, the
 * sole writer of `diffImagePath`. Snap provider summaries carry `comparison`
 * metadata without a rendered diff image, so the legend is gated on an
 * actual embedded image, not comparison metadata.
 *
 * @param {DriftSummary['changed'][number]} item
 * @returns {boolean}
 */
