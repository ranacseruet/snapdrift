...
/**
 * A semantic palette diff image (orange/green/red) is only generated by the
 * local adapter, which is the sole writer of `diffImagePath`; Snap provider
 * summaries carry `comparison` metadata without a rendered diff image. The
 * full palette additionally requires unequal dimensions, the only condition
 * under which one-sided (added/removed) pixels can exist.
 *
 * @param {DriftSummary['changed'][number]} item
 * @returns {boolean}
 */
function hasSemanticDiffImage(item) {
  return Boolean(item.diffImagePath && item.comparison?.dimensionsChanged);
}
...
export async function generateHtmlReport(summary, options = {}) {
  ...
  const semanticDiff = summary.changed.some(hasSemanticDiffImage);
  ...
  if (dimensionShiftCount === 0) {
    ...
    </table>${comparisonDimensionChanges.some(hasSemanticDiffImage) ? `<p class="diff-legend"><sub>${escapeHtml(DIFF_IMAGE_LEGEND)}</sub></p>` : ''}`;
  }
