 ...
 * Images are embedded as base64 data URIs when an imageReader is provided.
 *
  ...
  if (summary.changed.length === 0) {
    ...
    const rows = await Promise.all(summary.changed.map(async (item) => {
      ...
      if (imageReader && baselineRunDir) {
        ...
        baselineImgHtml = imgTag(base64, `Baseline: ${item.id}`);
      }
      if (imageReader && currentRunDir) {
...
<body>
  ...
  <footer>Generated by <a href="${DEFAULT_SNAPDRIFT_REPO_URL}">SnapDrift</a> &middot; ${generatedAt}</footer>
</body>
</html>
`;
}