...
Hosted PR diffs use the same persisted `expectedCaptures` identities. `SnapProvider.diff()` requires a nonempty, unique expected set consistent with `selectedRouteIds`; results written by older SnapDrift versions without that set must be recaptured. It polls until the expected capture count is available, verifies the returned run id, and reconciles every returned capture by route id, path, and normalized viewport descriptor. Missing expected captures, duplicate or unexpected identities, pending or unknown statuses, run or capture errors, missing current objects, and invalid `diffPct` values produce an `incomplete` summary and are never counted as clean matches. A valid comparison requires a current object, a baseline object, and a finite mismatch ratio from 0 through 1. The `diff.mode` setting then controls whether that incomplete summary fails the check.
